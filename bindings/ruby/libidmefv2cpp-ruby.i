...
static VALUE __libidmefv2_log_func = Qnil;
...
static void _cb_ruby_log(int level, const char *str)
{
        ...
        rb_funcall(__libidmefv2_log_func, cid, 2, SWIG_From_int(level), SWIG_FromCharPtr(str));
}


static int _cb_ruby_write(libidmefv2_msgbuf_t *fd, libidmefv2_msg_t *msg)
{
        ssize_t ret;
        rb_io_t *fptr;
        VALUE *io = (VALUE *) libidmefv2_msgbuf_get_data(fd);

        GetOpenFile(*io, fptr);

        ret = fwrite((const char *) libidmefv2_msg_get_message_data(msg), 1, libidmefv2_msg_get_len(msg), GetWriteFile(fptr));
...
static ssize_t _cb_ruby_read(libidmefv2_io_t *fd, void *buf, size_t size)
{
        ssize_t ret;
        rb_io_t *fptr;
        VALUE *io = (VALUE *) libidmefv2_io_get_fdptr(fd);
...
%extend LibIdmefv2::IDMEFV2 {
        ...
        LibIdmefv2::IDMEFV2 &operator >> (void *nocast_p) {
                self->_genericWrite(_cb_ruby_write, nocast_p);
                return *self;
        }
...
VALUE IDMEFV2ValueList_to_SWIG(TARGET_LANGUAGE_SELF self, const LibIdmefv2::IDMEFV2Value &value, void *extra)
{
        int ret;
        VALUE ary;
        ...
        for ( i = result.begin(); i != result.end(); i++ ) {
                ...
                if ( (*i).isNull() )
                        ...
                        ret = IDMEFV2Value_to_SWIG(self, *i, extra, &val);
                        if ( ret < 0 )
                                return Qnil;
                }
...
%init {
        ...
        if ( argc + 1 < 0 )
                throw LibIdmefv2Error("Invalid argc length");
