...
static libidmefv2_string_t *get_message_ident(libidmefv2_ident_t *ident)
{
        int ret;
        ...
        ret = libidmefv2_ident_generate(ident, str);
        if ( ret < 0 ) {
 ...
 * @size: New size.
 *
 * libidmefv2_realloc() changes the size of the memory block pointed by @ptr
...
{
        size_t i, j, len;
        libidmefv2_bool_t eol, has_data = FALSE, miss_eol=FALSE;

        while ( size > 1 ) {
                if ( ! fgets(buf, size, fd) )
                        return (has_data) ? 0 : libidmefv2_error(LIBIDMEFV2_ERROR_EOF);
        ...
        return libidmefv2_error_verbose(LIBIDMEFV2_ERROR_EINVAL, "buffer is too small to store input line");
}



/**
 * libidmefv2_read_multiline2:
 * @fd: File descriptor to read input from.
 * @line: Pointer to a line counter.
 * @out: Pointer to a #libidmefv2_string_t object where the line should be stored.
 *
 * This function handles line reading separated by the '\' character.
 *
...
int libidmefv2_read_multiline2(FILE *fd, unsigned int *line, libidmefv2_string_t *out)
{
        ...
        libidmefv2_string_clear(out);

        do {
                ret = libidmefv2_read_multiline(fd, line, buf, sizeof(buf));
                if ( ret < 0 && (r = libidmefv2_error_get_code(ret)) != LIBIDMEFV2_ERROR_EINVAL ) {
                        if ( r == LIBIDMEFV2_ERROR_EOF && ! libidmefv2_string_is_empty(out) )
                                ret = 0;

                        break;
                }

                r = libidmefv2_string_cat(out, buf);
                if ( r < 0 )
                        return r;
 ...
 *
 * The libidmefv2_hton64() function converts the 64 bits unsigned integer @val
...
int libidmefv2_get_gmt_offset(long *gmtoff)
{
        time_t t = time(NULL);
        return libidmefv2_get_gmt_offset_from_time(&t, gmtoff);
}
...
int _libidmefv2_load_file(const char *filename, unsigned char **fdata, size_t *outsize)
{
        ...
        if ( st.st_size == 0 ) {
                close(fd);
                return libidmefv2_error_verbose(libidmefv2_error_code_from_errno(EINVAL), "could not load '%s': empty file", filename);
        }
