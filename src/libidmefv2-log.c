...
static int do_log_print(libidmefv2_log_t level, const char *str);


static int debug_level = 0;
static int raise_abort_level = 0;
libidmefv2_bool_t raise_abort_set = FALSE;
static int log_level = LIBIDMEFV2_LOG_INFO;

static char *global_prefix = NULL;

static FILE *debug_logfile = NULL;
static libidmefv2_log_flags_t log_flags = 0;
static int (*global_log_cb)(libidmefv2_log_t level, const char *str) = do_log_print;
static void (*external_log_cb)(libidmefv2_log_t level, const char *str) = NULL;
...
static void syslog_win32(int priority, const char *log)
{
        ...
        if ( ! event_source )
                event_source = RegisterEventSource(NULL, TEXT("LibIdemfv2"));
...
static int do_log_syslog(libidmefv2_log_t level, const char *str)
{
        int slevel = LOG_INFO;

        if ( level == LIBIDMEFV2_LOG_CRIT )
                slevel = LOG_CRIT;

        else if ( level == LIBIDMEFV2_LOG_ERR )
                slevel = LOG_ERR;

        else if ( level == LIBIDMEFV2_LOG_WARN )
                slevel = LOG_WARNING;

        else if ( level == LIBIDMEFV2_LOG_INFO )
                slevel = LOG_INFO;

        else if ( level >= LIBIDMEFV2_LOG_DEBUG )
                slevel = LOG_DEBUG;
...
static inline FILE *get_out_fd(libidmefv2_log_t level)
{
        return (level < LIBIDMEFV2_LOG_INFO) ? stderr : stdout;
}
...
static int do_log_external(libidmefv2_log_t level, const char *str)
{
...
static inline libidmefv2_bool_t need_to_log(libidmefv2_log_t level, libidmefv2_log_t cur)
{
        return (level > cur) ? FALSE : TRUE;
}


static const char *level_to_string(libidmefv2_log_t level)
{
        if ( level >= LIBIDMEFV2_LOG_DEBUG )
                return "DEBUG";
        ...
        else if ( level == LIBIDMEFV2_LOG_CRIT )
                return "CRITICAL";
...
static ssize_t get_header(libidmefv2_log_t level, char *buf, size_t size)
{
        int ret;
        struct tm *t;
        size_t len = 0;
        ...
        if ( ! (log_flags & LIBIDMEFV2_LOG_FLAGS_SYSLOG) ) {
                t = localtime(&now);
                if ( t )
...
static void do_log_v(libidmefv2_log_t level, const char *file,
                     const char *function, int line, const char *fmt, va_list ap)
{
        ...
        if ( level <= LIBIDMEFV2_LOG_ERR || level >= LIBIDMEFV2_LOG_DEBUG ) {
        ...
        if ( need_to_log(level, log_level) || need_to_log(level, debug_level) ) {
                ...
                if ( ret < 0 && global_log_cb == do_log_print ) {
                        libidmefv2_log_set_flags(LIBIDMEFV2_LOG_FLAGS_SYSLOG);
...
/**
 * _libidmefv2_log:
 * @level: LIBIDMEFV2_LOG_PRIORITY_INFO or LIBIDMEFV2_LOG_PRIORITY_ERROR.
 ...
 * Use the #log macro defined in libidmefv2-log.h
 */
...
/**
 * libidmefv2_log_set_flags:
 * @flags:
 */
void libidmefv2_log_set_flags(libidmefv2_log_flags_t flags)
{
        if ( flags & LIBIDMEFV2_LOG_FLAGS_QUIET )
                log_level = LIBIDMEFV2_LOG_WARN;

        if ( flags & LIBIDMEFV2_LOG_FLAGS_SYSLOG )
...
{
        debug_level = LIBIDMEFV2_LOG_DEBUG + level;
}



/**
 * libidmefv2_log_set_prefix:
 * @prefix: Pointer to the prefix to use.
 *
 * Tell the LibIdemfv2 standard logger to add @prefix before logging
 * a line.
 */
...
/**
 * libidmefv2_log_set_callback:
 * @log_cb: Callback function.
 *
 * @log_cb() will be called in place of the libidmefv2 function usally
 * used for loging.
 */
...
int _libidmefv2_log_set_abort_level_from_string(const char *level)
{
        ...
        struct {
                const char *prefix;
                libidmefv2_log_t level;
        } tbl[] = {
                { "CRIT",  LIBIDMEFV2_LOG_CRIT  },
                { "ERR",   LIBIDMEFV2_LOG_ERR   },
                { "WARN",  LIBIDMEFV2_LOG_WARN  },
                { "INFO",  LIBIDMEFV2_LOG_INFO  },
                { "DEBUG", LIBIDMEFV2_LOG_DEBUG }
        };

        libidmefv2_return_val_if_fail(level != NULL, libidmefv2_error(LIBIDMEFV2_ERROR_ASSERTION));

        for ( i = 0; i < sizeof(tbl) / sizeof(*tbl); i++ ) {
                if ( strncasecmp(tbl[i].prefix, level, strlen(tbl[i].prefix)) == 0 ) {
                        _libidmefv2_log_set_abort_level(tbl[i].level);
                        return 0;
                }
        }
        ...
        if ( eptr != (level + strlen(level)) || lvalue == LONG_MIN || lvalue == LONG_MAX ) {
                libidmefv2_log(LIBIDMEFV2_LOG_WARN, "Invalid abort level specified: '%s'.\n", level);
                return -1;
        }

        _libidmefv2_log_set_abort_level(lvalue);
...
void libidmefv2_log(libidmefv2_log_t level, const char *fmt, ...)
{
        ...
        va_start(ap, fmt);
        _libidmefv2_log_v(level, "", "", 0, fmt, ap);
        va_end(ap);
}
...
void libidmefv2_log_debug(libidmefv2_log_t level, const char *fmt, ...)
{
        ...
        va_start(ap, fmt);
        _libidmefv2_log_v(LIBIDMEFV2_LOG_DEBUG + level, "", "", 0, fmt, ap);
        va_end(ap);
}
