...
errnos_body {
  ...
  if ($1 !~ /^[0-9]/)
    {
# Note that this assumes that libidmefv2-error.h.in doesn't start with a digit.
      errnos_body = 0;
      libidmefv2_error_h = 1;
    }
  else
    {
      errnos_idx[errnos_nr] = "LIBIDMEFV2_ERROR_SYSTEM_ERROR | " $1;
      errnos_sym[errnos_nr] = "LIBIDMEFV2_ERROR_" $2;
      errnos_nr++;
    }
}
