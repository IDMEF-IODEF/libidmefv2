...
/* Return a pointer to a string containing the name of the symbol of
   ...
   code is not known.  */
const char *libidmefv2_strsource_sym(libidmefv2_error_t err)
{
  	libidmefv2_error_source_t source = libidmefv2_error_source(err);

  	if ( msgidxof(source) == msgidxof(LIBIDMEFV2_ERROR_SOURCE_DIM) )
    		return NULL;
