...
/* Return a pointer to a string containing the name of the symbol of
   ...
   code is not known.  */
const char *libidmefv2_strerror_sym(libidmefv2_error_t err)
{
	int idx;
  	libidmefv2_error_code_t code = libidmefv2_error_get_code(err);
  	...
  	if ( msgidxof(code) == msgidxof(LIBIDMEFV2_ERROR_CODE_DIM) )
    		return NULL;
