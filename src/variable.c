...
static LIBIDMEFV2_LIST(variable_list);
...
static variable_t *search_entry(const char *variable)
{
        ...
        libidmefv2_list_for_each(&variable_list, tmp) {
                item = libidmefv2_list_entry(tmp, variable_t, list);
...
static int create_entry(const char *variable, const char *value)
{
        ...
        libidmefv2_list_add_tail(&variable_list, &item->list);
...
{
        variable_t *item;
        libidmefv2_list_t *tmp, *bkp;

        libidmefv2_list_for_each_safe(&variable_list, tmp, bkp) {
                item = libidmefv2_list_entry(tmp, variable_t, list);
                destroy_variable(item);
        }
}
