...
class Names(object):
    ...
    def type_name(self):
        ...
        """
        return "_".join((self.name_lib, self.lower_name, "t"))
...
class Type(Enum):
    ...
    STRUCT = {"c_type": "struct", "data_type": DataType.STRUCT}
...
class Field(object):
    def __init__(self, name_lib="idmefv2", name_class="ClassName", type="typeOnJson", json_name="NameField", multiplicity=None, id=0, values=None, key_listed=False, enum_id=0, doc=None):
    ...
    @property
    def listed(self):
        return self.multiplicity is not None and self.multiplicity.listed
    ...
    def is_op_int(self):
        ...
        return self.multiplicity.optional and\
                (self.data_type == DataType.PRIMITIVE\
                        or (self.data_type == DataType.ENUM and all('0' != member["rank"] for member in self.values)))
...
class Struct(object):
    def __init__(self, name_lib="idmefv2", name="class", doc="", attributes=None, children=None, aggregates=None, ident=0):
        self.name_lib = name_lib
        self.name = name
        self.doc = doc
        if attributes == None:
...
class LibData(object):
    ...
    def unique_id(self):
        ...
        for obj in self.obj_list:
            ...
            if not used_id[obj.ident]:
                ...
                Check the corresponding files on %s_json directory" %\
                (obj.type_name, objs_by_id[obj.ident], self.name)
                return False
    ...
    def process(self, dict_class_list):
        for dict_class in dict_class_list:
            ...
            struct.is_pre_dec_struct(
                self.structs, self.predeclared_list, self.obj_list, self.predeclared)
